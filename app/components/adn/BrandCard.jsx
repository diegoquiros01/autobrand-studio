...
export default function BrandCard({ profile, dnaStrength = 0, en = false }) {
  ...
  const emptyPlaceholder = (text) => (
    <div style={{ border: '1px dashed rgba(255,255,255,0.12)', borderRadius: 8, padding: '8px 12px', fontSize: 11, color: 'rgba(255,255,255,0.25)', fontStyle: 'italic' }}>
      {text}
    </div>
  );

  return (
    <div style={{ background: '#16162D', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 16, overflow: 'hidden', boxShadow: dnaStrength >= 91 ? '0 0 20px rgba(64,192,87,0.12)' : 'none', transition: 'box-shadow 0.6s ease' }}>
      ...
      <div style={{ padding: '20px 24px' }}>
        ...
        <div style={{ marginBottom: 16 }}>
          ...
          {audiencia ? (
            <div style={{ fontSize: 12, color: t.text, lineHeight: 1.5 }}>{audiencia}</div>
          ...
          {coloresMarca.length > 0 ? (
            <div style={{ display: 'flex', gap: 6 }}>
              {coloresMarca.slice(0, 6).map((c, i) => (
                <div key={i} style={{ width: 24, height: 24, borderRadius: '50%', background: c, border: '1px solid rgba(255,255,255,0.1)' }} />
              ))}
            </div>
        ...
        {dnaStrength >= 91 && (
          <div style={{ marginTop: 16 }}>
            <button style={{ width: '100%', padding: 10, background: 'linear-gradient(135deg, #40C057, #2F9E44)', color: '#fff', border: 'none', borderRadius: 10, fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>
