    ...
    <div style={{ minHeight:"100vh", background:"#FAFAFA", fontFamily:"Inter, sans-serif" }}>
      <nav style={nav}>
        ...
        <div style={{ display:"flex", gap:8, marginLeft:"auto", alignItems:"center" }}>
          <button onClick={() => router.push("/")} style={{ padding:"7px 14px", borderRadius:8, fontSize:13, color:"#555", cursor:"pointer", background:"none", border:"none" }}>Generador</button>
      ...
      <div style={{ maxWidth:900, margin:"0 auto", padding:"40px 28px" }}>
        <div style={{ marginBottom:32, display:"flex", alignItems:"flex-start", justifyContent:"space-between" }}>
          <div>
            <h1 style={{ fontSize:26, fontWeight:500, color:"#0A0A0A", marginBottom:6, letterSpacing:"-0.02em" }}>Biblioteca de assets</h1>
            <p style={{ fontSize:14, color:"#888" }}>Tus fotos, logos y referencias visuales. AiStudioBrand los usa al generar contenido.</p>
          </div>
          <button
        ...
        >
          {uploading ? (
            ...
            <div>
              ...
              <div style={{ fontSize:14, color:"#555", fontWeight:500 }}>Arrastra tus fotos aquí o haz clic para seleccionar</div>
              <div style={{ fontSize:12, color:"#999", marginTop:4 }}>PNG, JPG, WEBP · Múltiples archivos</div>
            </div>
          )}
        </div>
        ...
        {loading ? (
          <div style={{ textAlign:"center", padding:"40px 0", color:"#999" }}>Cargando assets...</div>
        ) : assets.length === 0 ? (
          <div style={{ textAlign:"center", padding:"60px 0" }}>
            <div style={{ fontSize:40, opacity:0.15, marginBottom:12 }}>◉</div>
            <div style={{ fontSize:15, color:"#888", fontWeight:500 }}>No tienes assets todavia</div>
            <div style={{ fontSize:13, color:"#bbb", marginTop:6 }}>Sube tus fotos, logos y referencias para que AiStudioBrand los use al generar contenido</div>
          </div>
        ) : (
          <div>
            <div style={{ fontSize:12, fontWeight:500, color:"#999", textTransform:"uppercase", letterSpacing:"0.1em", marginBottom:16 }}>{assets.length} assets</div>
            <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill, minmax(160px, 1fr))", gap:12 }}>
              {assets.map((asset, i) => (
                <div key={i} style={{ background:"#fff", border:"1.5px solid #EAEAEA", borderRadius:12, overflow:"hidden", position:"relative" }}>
                  <img src={asset.url} alt={asset.name} style={{ width:"100%", aspectRatio:"1", objectFit:"cover", display:"block" }} />
                  <div style={{ padding:"10px 12px", display:"flex", alignItems:"center", justifyContent:"space-between" }}>
                    <div style={{ fontSize:11, color:"#888", overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap", maxWidth:100 }}>{asset.name}</div>
                    <button
